...
%Although the nearest-neighbor method provides a reasonable estimate at large scales, in Section \ref{sec:fc_pk} we demonstrate that at small scales its effects on the power spectrum are significant and dominate sample variance. In order to correct for this effect, in Section \ref{sec:dlospeak} we present an improved method to account for correlated fiber collided galaxies that uses the line-of-sight displacement between fiber collided pairs (Section \ref{sec:dlos}) and a shot-noise correction term in the power spectrum estimator that properly takes into account fiber collisions (Section \ref{sec:shotnoise}). In Section \ref{sec:fourier}, we present a complementary method that models the effect of fiber collisions on the power spectrum using an analytic estimate of the effect of fiber collisions on the 2PCF. 
\begin{figure*}
\begin{center}
...
% Not only quantify the effect of fiber collisions, but the break down of the NN method for P(k) at high k. 
