...
\begin{table*} 
  \caption{Galaxy Subsamples} \label{tab:subsample}
  \begin{center}
    \leavevmode
...
\begin{figure}
  \begin{center}
    \leavevmode
    \epsfig{file=fig_envcount_cylr2_5h35_thresh75_sdss_z0_05_0_12_primus_z0_2_1_0_lowenv0_5_highenv3_0_primuszerr_masslimit_edgecut_histogram.png, width=\textwidth}
...
\begin{figure*}
  \begin{center}
    \leavevmode
    \epsfig{file=fig_smf_cylr2_50000h35_thresh75_bin0_20_sdss_z0_05_0_12_primus_z0_2_1_0_2envbin_lowenv0_5_highenv3_0_lit_primuszerr.eps, height=\textwidth,angle=90}
...
\begin{figure*}
    \begin{center}
        \leavevmode
        \epsfig{file=fig_qf_cylr2_50000h35_thresh75_bin0_20_sdss_z0_05_0_12_primus_z0_2_1_0_lowenv0_5_highenv3_0_lit_primuszerr.eps, height=\textwidth,angle=90}
...
\begin{center}
\epsscale{0.8}
\epsfig{file=fig_qffit_cylr2_50000h35_thresh75_bin0_20_sdss_z0_05_0_12_primus_z0_2_1_0_lowenv0_5_highenv3_0_fidmasscomp11_0_10_5_lit_primuszerr.eps, height=0.8\textwidth}
\caption{The evolution of the quiescent fraction at fiducial mass, $f_{Q}(\mathcal{M}_{\mathrm{fid}})$, for low (blue) and high (red) density environments within the redshift range $z = 0.0 - 0.8$. We present the $f_{Q}(\mathcal{M}_{\mathrm{fid}})$ evolution for $\mathcal{M}_{\mathrm{fid}} = 10^{10.5} \mathcal{M}_\odot$ (solid fill) and $10^{11} \mathcal{M}_\odot$ (patterned fill) with the uncertainty of the best-fit parameter $b$ in Equation \ref{eq:qffit} represented by the width of the line. While the high density $f_{\mathrm{Q}}(\mathcal{M}_{\mathrm{fid}})$ is greater than low density environment $f_{\mathrm{Q}}(\mathcal{M}_{\mathrm{fid}})$ over the entire redshift range of our sample, there is a significant increase in $f_{\mathrm{Q}}(\mathcal{M}_{\mathrm{fid}})$ over cosmic time for both environments. For the environment cut-offs ($n_{\mathrm{env}} < 0.5$ for low and $n_{\mathrm{env}} > 3.0$ for high), there is no significant difference in the slope of the evolution between the environments.}         \label{fig:qffit}
\end{center}
\end{figure}
...
$0.6 < z < 0.8$      &$n_{\mathrm{env}} < \lowenvthresh$ & $0.284 \pm 0.036$ & $0.352 \pm 0.019$ \\
  ...
  \multicolumn{4}{l}{}                                             \\       
    \end{tabular} \par
    \end{center}
	\tablecomments{Best fit parameters in Equation \ref{eq:qffit} for each subsample $f_{\mathrm{Q}}(\mathcal{M}_{*})$ in Figure \ref{fig:qf} for $\mathcal{M}_{\mathrm{fid}} = 10^{10.5} \mathcal{M}_{\odot}$.}
\end{table}
\clearpage
