...
For our aperture, we use a cylinder of dimensions: $R_{\rm{ap}} = 2 \;
h^{-1} \rm{Mpc}$ and $H_{\rm{ap}} = 25 \; h^{-1}\rm{Mpc}$.  We use a
...
corresponds to $\sigma_z < 0.01$, so our aperture height choice of $25
\; \rm{Mpc}/h$ accounts for both of these effects. Our choice of
...
dimensions $R_{\rm{ap}} = 2 \:h^{-1}\rm{Mpc}$ and $H_{\rm{ap}} = 25
\:h^{-1}\rm{Mpc}$ to measure environment. The same analysis was
extended for varying aperture dimensions $R_{\rm{ap}} = 1, 2, 3 \:
h^{-1}\rm{Mpc}$ and $H_{\rm{ap}} = 25, 50 \:h^{-1}\rm{Mpc}$. We also
