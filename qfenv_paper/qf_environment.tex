...
%Compare SMFs at low environment vs high environment and remark that the SMFs are significantly different. 
%Remark on the clear mass-density relation: more massive galaxies in high environment 
% Compare to Bolzonella et al. 2010 and perhaps even Bundy et al. 2006 results
% Agree with previous works on mass segregation results from VVDS, DEEP2, and etc. 

\section{Quiescent Fraction} \label{sec:qf_const}
The SMFs calculated in the previous section illustrate the stellar mass distribution of our galaxy population and its evolution over comic time. In this section, using the SMFs of our subsamples, we compare the quiescent and the star-forming populations using the quiescent fraction - the fraction of galaxies that have depleted their star-formation. With the sufficient statistics available from SDSS and PRIMUS, we evaluate the quiescent fraction in bins of stellar mass and redshift for low and high density environments. More specifically, by analyzing the quiescent fraction with respect to mass, redshift, and environment we are able to compare the quiescent fraction evolution over redshift for galaxies in low and high density environments. Through this comparison we reveal the subtle environmental effects in quenching star formation that are often obscured by the underlying correlations among galactic properties such as color-mass and mass-density relations (\cite{Cooper:2010aa}). Furthermore by quantifying the environmental effects of star-formation quenching, we are able to provide constraints on the various proposed quenching mechanisms, such as strangulation and ram-pressure stripping (CITECITE), that operate on the scale of our environment measure.
...
Most noticeably, $f_{\rm{Q}}$ increases monotonically as a function of mass regardless of redshift bin or environment. In other words, for all subsamples, galaxies in higher mass bins are more likely to have quenched its star-formation. With the roughly linear correlation between galaxy SFR to galaxy color and morphology, we find that this trend reflects the well established color-mass and morphology-mass relations: more massive galaxies are more likely to be red or early-type (\cite{Blanton:2009aa}). 

Focusing on the redshift evolution of $f_{\rm{Q}}$, we find that for both environments and over our mass range $f_{\rm{Q}}$ increases as redshift decreases. For high density environment, this is analogous to the Butcher-Omeler Effect (\cite{Butcher:1984aa}), which states that galaxy populations in groups or clusters have higher $f_{\rm{blue}}$, or lower $f_{\rm{Q}}$, at higher redshift. For all environments a larger portion of our galaxy population has quenched its star-formation over time. Coupled with the decline of SFR in the star-forming population (\cite{Noeske:2007aa}; \cite{cooper08a}), our results strongly support the observed decline of global star-formation in the universe (\cite{hopkins06a}). 

Also, by comparing the stellar masses at which $f_{\rm{Q}} = 0.5$ for each subsample, we find that this mass decreases over cosmic time. This corresponds to the well-known mass-downsizing pattern (\cite{bundy06a}). Furthermore, the mass-downsizing trend observed in each of our environment subsample is generally consistent with the trend observed in zCOSMOS Redshift Survey for isolated and group galaxies (\cite{Iovino:2010aa}). 

We compare our $f_{\rm{Q}}$ results to previous $f_{\rm{Q}}$ or $f_{\rm{red}}$ measurements for analogous environment classifications in literature. For our lowest redshift bin, we find that our $f_{\rm{Q}}$ values for low and high environments are consistent with previous $f_{\rm{Q}}/f_{\rm{red}}$ measurements for SDSS galaxies as a function of environment. For example, \cite{Tinker:2011aa}, using a group-finding algorithm on the SDSS DR7, presents the relationship between $f_{\rm{Q}}$ and overdensity for galaxies within the mass range $\rm{log} \; \mathcal{M}_{*} = [9.8, 10.1]$. The \cite{Tinker:2011aa} $f_{\rm{Q}}$ values at the lowest and highest overdensity values, $f_{\rm{Q}} \sim 0.4$ and $f_{\rm{Q}} \sim 0.6$ respectively, are consistent with our SDSS sample low and high density environment $f_{\rm{Q}}$ at the lowest mass bin $\rm{log} \; \mathcal{M} \sim 10.2$. At higher mass ranges, our $f_{\rm{Q}}$ results for low and high density environments are consistent with \cite{Baldry:2006aa} $f_{\rm{red}}$ values for lowest and highest environment density, which they measure using projected neighbor density. 

At higher redshifts, we compare our PRIMUS $f_{\rm{Q}}$ results to the $f_{\rm{red}}$ (or $1-f_{\rm{blue}}$) results from the zCOSMOS Redshift Survey (\cite{Iovino:2010aa}, \cite{Cucciati:2010aa}, \cite{Kovac:2014aa}), which roughly covers the same redshift range as PRIMUS. \cite{Iovino:2010aa}, \cite{Cucciati:2010aa}, and \cite{Kovac:2014aa} using a mass-complete galaxy sample derived from zCOSMOS and environment determined using a group catalog, 3D local density contrast, and overdensity respectively, compare $f_{\rm{red}}$ for different environments. 

The $f_{\rm{blue}}$ values for group and isolated galaxies from \cite{Iovino:2010aa} are generally inconsistent with our $f_{\rm{Q}}$ values in high and low density environments. Similarly, the $f_{\rm{red}}$ values for high and low overdensities from \cite{Kovac:2014aa} disagree with our $f_{\rm{Q}}$ values in high and low density environments. 


Also, the zCOSMOS results, which are mostly consistent with each other, indicate that the $f_{\rm{red}}$ values in high density environments are greater than in low density environments. The same trend is observed in the PRIMUS results and will be discussed in detail later in the section. 
Furthermore, \cite{Iovino:2010aa} in their analysis finds clear mass dependence in the $f_{\rm{red}}$ evolution where $f_{\rm{red}}$ evolves by a greater amount for lower mass galaxies in both group and isolated galaxies. 

 %explain the zCOSMOS results briefly. Mention the general discrepancies while highlighting the large statistics offered by PRIMUS
%If we impose the f_Q vs f_red discrepancies plotting in Kovac et al. 2014, can we explain the discrepancies between Iovino et al. 2010 and PRIMUS? Is Iovino et al. 2010 and Kovac et al. 2014 consistent with one another? 
%Note the clear mass dependence in essentially all the zCOSMOS results 
%color-density relation?

In contrast to the zCOSMOS results, for each panels in Figure \ref{fig:qf}, qualitatively, we see little mass dependence on the $f_{\rm{Q}}$ evolution. However, due to the distinct stellar mass completeness limits imposed at each redshift bin of our sample, we require a more quantitative comparison of the $f_{\rm{Q}}$ redshift evolution. Consequently, we fit each subsample $f_{\rm{Q}}$ to a power-law parameterization as a function of stellar mass, 
...
The same fitting was performed using $\mathcal{M}_{\rm{fid}} = 10^{11} \mathcal{M}_\odot$; however, aside from a general shift in all $f_{\rm{Q}}(\mathcal{M}_{\rm{fid}})$ values by roughly $\sim 0.2$, we observe the same evolutionary trends. 

From Figure \ref{fig:qffit}, we note that for both environments $f_{\rm{Q}}(\mathcal{M}_{\rm{fid}})$ increases as redshift decreases. This highlights the general increase of $f_{\rm{Q}}(\mathcal{M})$ as redshift decreases noted earlier. More importantly it quantifies the total evolution of $f_{\rm{Q}}$ since $z \sim 0.8$ for both low and high density environments. Remarkably, the total evolution of $f_{\rm{Q}}$ remains constant for both environments. In other words, the total evolution of $f_{\rm{Q}}(\mathcal{M}_{\rm{fid}})$ from $z \sim 0.7$ to $\sim 0.1$ show no strong environment dependence. 

%Discuss how this is not due to contamination by environment classifications. 

In addition, throughout the redshift range, high density environment has a significantly greater $f_{\rm{Q}}(\mathcal{M}_{\rm{fid}})$ than the low density environment. 

%PRIMUS results are roughly in agreement with zCOSMOS 10k (Iovino et al. 2010) and zCOMOS 20k (Kovac et al. 2014) once the difference between f_red and f_Q are taken into account. Discuss some general specific values. 
% Previous results, primarily from zCOSMOS 10k and 20k, have found a significant mass dependence in the f_{Q} evolution with f_{Q} increasing significantly more over redshift at lower masses. At higher masses these results find little evolution in f_Q. Though our f_{Q} values are in good agreement with these results, for the mass ranges probed in our analysis, we find little mass dependence in the f_Q evolution. 

% color-mass relation, mass-density relation, color-density relation, SFR-density relation, SSFR-density relation, 

% The "color-mass" relation holds for both low and high environment galaxies. While others in literature (Cucciati et al. 2010) found that the mass dependence was less apparent in high environment, this does not hold true for the SDSS and PRIMUS data. Although this could be due to the contamination in our environment measurements since there are large redshift errors. 
% Emphasize that due to the limited mass range from mass completeness limits, the conclusions on mass dependence should be taken with a grain of salt. 
% The redshift evolution of the QF in each 
